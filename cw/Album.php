...
<?php 
    ...
    {
       $del = $_GET['del'];       
       $sql="DELETE FROM CD WHERE cdID=$del;";
    ...
    $no_rows = mysqli_num_rows($result);
       ...
       if ($no_rows>0) { // if there is anything in the database
    	...
    	echo "<tr><th>cdID</th><th>artID</th><th>cdTitle</th><th>cdPrice</th><th>cdGenre</th><th> Edit </th><th>Delete</th><th></th></tr>"; // table header
    	...
    	foreach ($result as $row) // loop through result array
    	{
          ...
          // Delete button executes JavaScript confirmDelete function  
			 echo '<td><a href="editAlbum.php?id='.$row['cdTitle'].'"><center><button type = "button"><b>Edit</b></button></center></td></a>';           
          echo "<td><button onclick=confirmDelete($id,'$cdID')>Delete</button></td>"; 
  ...
  ?> 
  <input type="button" value="ADD NEW DATA" onclick="window.location.href='newalbum.php'" />
</article>
