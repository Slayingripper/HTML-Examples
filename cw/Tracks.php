...
<header>
  ...
  <script>
    ...
    function confirmDelete(trackID,name)
    {
        var conf = confirm("Are you sure you want to delete "+name+"?");
        if (conf == true) // if OK pressed
        {
            delParam="?del="+trackID; // add del parameter to URL
...
<?php 
    ...
    if ($no_rows>0) { // if there is anything in the database
    	...
    	echo "<tr><th>cdID</th><th>artID</th><th>trackID</th><th>TrackName</th><th>Edit</th><th>Delete me</th></tr>"; // table header
    	...
    	foreach ($result as $row) // loop through result array
    	{
          ...
          // Delete button executes JavaScript confirmDelete function   	      	  
			 echo '<td><a href="editTrack.php?id='.$row['trackID'].'"><center><button type = "button"><b>Edit</b></button></center></td></a>';           
          echo "<td><button onclick=confirmDelete($id,'$trackID')>Delete</button></td>"; 
  ...
  ?> 
   <input type="button" value="ADD NEW DATA" onclick="window.location.href='newtrack.php'" />
</article>
